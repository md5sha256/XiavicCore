...
public class SignEditorCommand implements TabExecutor {
    ...
    public boolean onCommand(@NotNull final CommandSender sender, @NotNull final Command command,
        ...
        if (sender.hasPermission(permissions.getString("SignEdit")) || sender.isOp()) {
            if (sender instanceof Player) {
                ...
                if (lookingAt == null || !(lookingAt.getState() instanceof Sign)) {
                    if (args.length > 0) {
                        ...
                        String toggleMessage = messages.getString("DynamicSignEditToggled");
                        ...
                        if (param0.equalsIgnoreCase("on")) {
                            ...
                            Utils.chat(player, messages.getString("NotLookingAtSign"));
                            return true;
                        }
                        ...
                        Utils.chat(player, messages.getString(messageKey));
                    }
                    return true;
                }
                ...
                if (args.length < 1) {
                    if (iSignEditor.isDynamicEditingEnabled(player.getUniqueId())) {
                        ...
                        Utils.chat(player, messages.getString("SpecifyTarget"));
                        return true;
                    }
                }
                String toggleMessage = messages.getString("DynamicSignEditToggled");
                boolean isArgs0Line = false;
                switch (args[0]
                ...
                if (line < 1 || line > 4) { //If lines are out of bounds.
                    String invalidArgs = messages.getString("InvalidArgs");
                ...
                String[] lines =
                    parseSignInput(Arrays.copyOfRange(args, isArgs0Line ? 0 : 1, args.length)); //Check if first arg was a line-identifier.
                if (lines.length > 5 - line) { //5 = MaxLines (4) + (User readbility (1) - line number = Max additional lines.
                    Utils.chat(player, messages.getString("SignTooManyLines")
                        .replace("%lines%", String.valueOf(lines.length)));
                    return true;
                }
                for (String str : lines) { //Check for char overflow.
                    if (str.length() > 15) {
                        Utils.chat(player, messages.getString("SignLineCharOverflow")
                ...
                Utils.chat(player,
                    messages.getString(sign.update(true) ? "SignEdited" : "SignEditFailed"));
            } else {
                Utils.chat(messages.getString("SenderNotPlayer"));
                return true;
            }
        } else {
            Utils.chat(messages.getString("NoPerms"));
        }
        return true;
    }
